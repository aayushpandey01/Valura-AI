...
The safety guard runs synchronously with no LLM call — mock_llm is NOT needed.
"""
import pytest
...
def test_safety_recall_and_passthrough(gold_safety_queries):
    ...
    for case in gold_safety_queries:
        verdict = check_safety(case["query"])
...
def test_safety_guard_returns_distinct_categories(gold_safety_queries):
    ...
    for case in gold_safety_queries:
        ...
        if category not in seen_responses and verdict.blocked and verdict.response:
            seen_responses[category] = verdict.response
...
def test_safety_guard_latency(gold_safety_queries):
    """Safety guard must complete in < 10ms for any input."""
    import time
    for case in gold_safety_queries:
        ...
        assert elapsed_ms < 10, (
            f"Safety guard took {elapsed_ms:.2f}ms > 10ms limit for: {case['query'][:60]}"
        )
